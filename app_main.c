...
char topic1[50] = "esp32/BLE2MQTT";
char topic2[50] = "esp32/MQTT2BLE";
char myData[128];
char data[256];
int qos = 1;
...
static uint8_t char_value[10] = {0x11, 0x22, 0x33, 0x44};
    ...
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->read.handle) {
                ESP_LOGE(EXAMPLE_TAG, "(2) ***** read char ***** ");
            }
       	    break;
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle) {
                    ...
                    sprintf(data, "%s", param->write.value);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        esp_mqtt_client_subscribe(client, topic2, qos);
        break;
